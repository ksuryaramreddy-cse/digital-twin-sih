...
export default function EnergySection({ energy, onUpdate }) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              Diesel Microgrid Gensets • 320 kWh Battery Storage Bank • 415V 3-Phase Bus
            </p>
          </div>
        </div>
      ...
      {/* 4 Core Energy Cards: Battery Level, Power Consumption, Generator Status, Generator Load */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          unit="kW"
          sublabel={`Capacity: ${energy.powerGenerationKW} kW (${energy.dailyEnergyKWh} kWh/day)`}
          icon={Zap}
          trend="up"
          trendValue="+2.4 kW / 2h"
          color="amber"
        ...
        <MetricCard
          ...
          unit=""
          sublabel="GEN-01 Active • GEN-02 Hot Standby"
          icon={Cpu}
          color="cyan"
          statusText="3-Phase 50.08 Hz Synced"
        />
        ...
        <MetricCard
          ...
          unit="%"
          sublabel={`Grid: ${energy.gridVoltage}V AC • ${energy.gridFrequency} Hz`}
          icon={Gauge}
          trend="neutral"
          trendValue="Nominal 78%"
          color="sky"
          progress={energy.generatorLoadPct}
        />
      </div>

      {/* Detailed Genset Microgrid Matrix */}
      <div className="tactical-panel rounded-xl p-5 border border-cyan-500/20">
        <div className="flex items-center justify-between pb-3 mb-3 border-b border-polar-800">
          <div className="flex items-center space-x-2 text-xs font-mono font-bold text-slate-200 uppercase tracking-wider">
            ...
            <span>Maitri 3x 62.5 kVA Diesel Generator Fleet Status</span>
          </div>
          <span className="text-xs font-mono text-emerald-400 font-semibold">1 RUNNING • 1 HOT STANDBY • 1 COLD</span>
        </div>
          ...
          {energy.generators?.map((gen) => (
            <div 
              ...
              className={`p-3.5 rounded-xl border font-mono transition ${
                gen.status === 'RUNNING'
                  ? "bg-polar-900/90 border-cyan-500/40 shadow-sm shadow-cyan-500/10"
            ...
            >
              ...
              <div className="mt-2.5 pt-2 border-t border-polar-800 flex justify-between text-[10px] text-slate-400">
                <span>Oil: {gen.oilPressureBar > 0 ? `${gen.oilPressureBar} bar` : '0 bar'}</span>
